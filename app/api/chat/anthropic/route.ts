...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    headers.set('cf-cache-ttl', '3600000'); // Cache for 1 hour (3600000 ms)
    // Use 'cf-skip-cache' if you need to bypass cache for some reason
    // headers.set('cf-skip-cache', 'true');

    try {
      const response = await fetch("https://gateway.ai.cloudflare.com/v1/77a0b1436313aeb84549202bdd962b63/pixelverseaisystems/anthropic/messages", {
        method: 'POST',
        headers: headers,
        body: JSON.stringify({
